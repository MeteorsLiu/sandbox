...
- The primary comparison is the full execution environment: Docker includes LLAR and its build tools; sandbox includes host LLAR, Sentry, guest and build tools; Firecracker includes the VMM, guest Linux, LLAR and build tools. All three exclude the same outer Docker daemon and benchmark controller. Guest-only estimates and windows ending before state.Save are diagnostics, not values in this comparison. Source revision, image identity, binary hashes and input hashes are retained.
...
- The controller samples raw Docker API `memory_stats.usage` with a 100 ms wait between rounds and sums active containers. This includes page cache, the sandbox host/Sentry/workers, or the Firecracker VMM and resident guest memory. The common Docker daemon is excluded. It is not the cache-subtracted Docker CLI display, a Go heap counter, or gVisor MemoryFile+Mapped accounting. Shared-page charge ownership follows cgroup accounting, not summed process RSS. Short peaks can be missed and reads across concurrent containers are not atomic; per-read times and memory breakdowns are retained. State Save/Load and exit GC remain inside the measurement window.
