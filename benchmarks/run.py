...
def main():
    ...
    (output / "environment.json").write_text(json.dumps({
        ...
        "memory_window": "Full execution environment lifetime, including preparation, state Save/Load, guest exit and GC; no host-memory subtraction",
    ...
    def task(backend, concurrency, count, task_id):
        ...
        record["workload"] = audit_workload((output / f"{name}.log").read_text(), count, record["events"])
        record["success"] = record["exit_code"] == 0 and not record["event_errors"] and not record["workload"]["errors"] and len(outcomes) == count and all(not e.get("error") for e in outcomes)
        with lock:
            active.remove(cid)
    ...
    for backend in args.backends.split(","):
        for concurrency in map(int, args.concurrency.split(",")):
            ...
            summary = {"backend": backend, "concurrency": concurrency, "count": count,
                       ...
                       "workload_errors": sum(len(r["workload"]["errors"]) for r in records),
            ...
            failures += count-success+bool(sampler_errors)+sum(r["exit_code"] != 0 for r in records)+summary["event_errors"]+summary["workload_errors"]
    if failures:
