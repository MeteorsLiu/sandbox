...
# Populate the guest filesystem; these are files, not host mounts.
for device in null:3 zero:5 random:8 urandom:9; do
  mknod -m 666 "$output/rootfs/dev/${device%:*}" c 1 "${device#*:}"
done
ln -s /proc/self/fd "$output/rootfs/dev/fd"
cp -a "$output/rootfs/opt/benchmark" /opt/benchmark
python3 - "$output" <<'PY'
